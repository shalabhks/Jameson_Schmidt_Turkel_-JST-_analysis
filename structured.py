...
def calc_d2u():
    ...
    for ii in range(2,nx+1):
        for jj in range(2,ny+1):
            ...
            mui = np.abs(nu)/(de+1.0e-6)
            nu = np.abs(U[ii,jj+1]) + np.abs(U[ii,jj-1]) - 2*np.abs(U[ii,jj])
            de = np.abs(U[ii,jj+1]) + np.abs(U[ii,jj-1]) + 2*np.abs(U[ii,jj])
            muj = np.abs(nu)/(de+1.0e-6)
            due = U[ii+1,jj] - U[ii,jj]
            duw = U[ii-1,jj] - U[ii,jj]
            dun = U[ii,jj+1] - U[ii,jj]
            dus = U[ii,jj-1] - U[ii,jj]
            d2u[i,j] = mui*(A*dy*due + A*dy*duw) + muj*(B*dx*dus + B*dx*dun)
            d2u[i,j] = k2*d2u[i,j]
...
def calc_d4u():
    ...
    for ii in range(2,nx+1):
        for jj in range(2,ny+1):
            ...
            de = np.abs(U[ii,jj+1]) + np.abs(U[ii,jj-1]) + 2*np.abs(U[ii,jj])
            muj = np.abs(nu)/(de+1.0e-6)
            ep4i = max(0,(k4-mui*k2))
            ep4j = max(0,(k4-muj*k2))
            due  = U[ii+2,jj] - 3*U[ii+1,jj] + 3*U[ii,jj] - U[ii-1,jj]
            duw  = U[ii+1,jj] - 3*U[ii,jj] + 3*U[ii-1,jj] - U[ii-2,jj]
            dun  = U[ii,jj+2] - 3*U[ii,jj+1] + 3*U[ii,jj] - U[ii,jj-1]
            dus  = U[ii,jj+1] - 3*U[ii,jj] + 3*U[ii,jj-1] - U[ii,jj-2]
            d4u[i,j] = ep4i*A*dy*(due - duw) + ep4j*B*dx*(dun - dus)
